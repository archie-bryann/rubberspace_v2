...
<legend><h1 align = "center">Send Feedback <i class = "material-icons">edit</i></h1></legend><br>
        <div class = "jumbotron">
 <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method = "POST">
    ...
    <button type="button" class="close" data-dismiss="alert">&times;</button><i class = "material-icons left">error</i> '.$report->makeReport($name, $telephone, $email, $title, $message, $user_id).'</div>';
        ...
        // alert
        if(isset($_GET['sent'])) {
        echo '<div class="alert alert-dismissible alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><i class = "material-icons left">check</i> Thanks for your feedback!</div>';
    }
...
echo '<input id = "email" required readonly name = "email" type="email" class="form-control" aria-describedby="emailHelp" placeholder="Enter email" value = "'.$results->displayEmail().'">';
} 
}else {
echo '<input id = "email" required  name = "email" type="email" class="form-control" aria-describedby="emailHelp" placeholder="Enter email">';
}
?>
</div>
...
<button name = "submit" type="submit" class="btn <?php echo button1;  ?> btn-lg btn-block">Send</button>
